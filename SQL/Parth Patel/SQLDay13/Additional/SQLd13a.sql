...
AS (  
        SELECT e.EmployeeID, e.FirstName, e.LastName, e.JobId, 0 
		FROM Employees e   
INNER JOIN Departments d   
ON e.EmployeeID = d.DepartmentID  
        WHERE e.EmployeeID = @EmpID  
        UNION ALL  
        SELECT e.EmployeeID,e.FirstName, e.LastName, e.JobId, RecursionLevel + 1 
		FROM Employees e   
            INNER JOIN EMP_cte  
            ON e.EmployeeID = EMP_cte.EmployeeID  
INNER JOIN Departments d    
ON e.EmployeeID = d.DepartmentID  
        )  
